...
<?php if(is_array($slides)): foreach($slides as $k=>$vo): if($k == 0): ?><img src="<?php echo sp_get_asset_upload_path($vo['slide_pic']);?>" alt=""><?php endif; endforeach; endif; ?>
  ...
  <div style="width:1110px; margin:0px auto; overflow:hidden;">
    <div id="example-one">
      <ul class="nav">
        <?php $caseLists = sp_sql_posts_paged_bycatid($cat_id,"order:post_date DESC;"); ?>
        <?php if(is_array($caseLists['posts'])): $i = 0; $__LIST__ = $caseLists['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="nav-one"><a href="<?php echo leuu('article/index',array('id'=>$vo['object_id'],'cid'=>$vo['term_id']));?>"><?php echo ($vo["post_title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
      </ul>
    </div>
  </div>
  <div class="main_case">
      ...
      <div class="prj_box clearfix">
        <div class="prj_box_r">
          <ul class="prj_list clearfix">
            <?php if(is_array($caseLists['posts'])): $i = 0; $__LIST__ = $caseLists['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li onclick="location='<?php echo leuu('article/index',array('id'=>$vo['object_id'],'cid'=>$vo['term_id']));?>'">
                <div class="prj_left_image"> <img src="/themes/simplebootx/Public/assets/picture/coin.png" /> </div>

...
<div class="clear"></div>
<div id="warp_main">
  <?php $slides=sp_getslide("project_ads"); ?>
...
<?php if(is_array($slides)): foreach($slides as $k=>$vo): if($k == 0): ?><img src="<?php echo sp_get_asset_upload_path($vo['slide_pic']);?>" alt=""><?php endif; endforeach; endif; ?>
  ...
  <div style="width:1110px; margin:0px auto; overflow:hidden;">
    <div id="example-one">
      <ul class="nav">
        <?php $projectLists = sp_sql_posts_paged_bycatid($cat_id,"order:post_date DESC;"); $projectListPages = sp_sql_posts_paged_bycatid($cat_id,"order:post_date DESC;",6); ?>
        <?php if(is_array($projectLists['posts'])): $i = 0; $__LIST__ = $projectLists['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="nav-one"><a href="<?php echo leuu('article/index',array('id'=>$vo['object_id'],'cid'=>$vo['term_id']));?>"><?php echo ($vo["post_title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
      </ul>
    </div>
  </div>
  <div class="main_case">
    <div class="case_con clearfix">
      <div class="case_list clearfix">
        <ul>
          <?php if(is_array($projectListPages['posts'])): $k = 0; $__LIST__ = $projectListPages['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k; $smeta=json_decode($vo['smeta'], true); ?>
            <li> <a href="<?php echo leuu('article/index',array('id'=>$vo['object_id'],'cid'=>$vo['term_id']));?>">
              ...
              <div class="zhi1"> <a href="<?php echo leuu('article/index',array('id'=>$vo['object_id'],'cid'=>$vo['term_id']));?>"><?php echo ($vo["post_title"]); ?></a> </div>
    ...
    <div class="pro_page">
      <div class="pager_pro">
        <ul>
          <?php echo ($projectListPages['page']); ?>
        </ul>
      </div>
    </div>
  </div>

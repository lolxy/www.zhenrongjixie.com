  ...
  <div class="main_banner">
   ...
   <div class="main_about"> 
     ...
     <div class="main_tit_l"> 
      <span>工程项目总包 / </span>
      <span class="pro_tit">PROJECT</span> 
     </div> 
     <div class="main_tit_r clearfix"> 
      <div class="home_tit" style="width:263px;"> 
    ...
    <div style="width:1110px; margin:0px auto; overflow:hidden;"> 
     <div id="example-one"> 
		  <ul class="nav">
        <?php $projectLists = sp_sql_posts_paged_bycatid($term_id,"order:post_date DESC;"); ?>
        <?php if(is_array($projectLists['posts'])): $i = 0; $__LIST__ = $projectLists['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li class="nav-one"><a href="<?php echo leuu('article/index',array('id'=>$vo['object_id'],'cid'=>$vo['term_id']));?>" <?php if($vo['object_id'] == $id): ?>class="current"<?php endif; ?>><?php echo ($vo["post_title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
      </ul>
	  </div>
    </div>
   
   <div class="new_about">
		<div class="new_about_bp">
        	<img src="<?php echo sp_get_asset_upload_path($smeta['thumb']);?>" />
        </div>
   		<div class="new_about_bluef"><?php echo ($post_title); ?></div>
        <div class="new_about_santu">
            <?php if(is_array($smeta['photo'])): foreach($smeta['photo'] as $key=>$vo): ?><div class="new_about_sp">
               <img width="296" height="204" src="<?php echo sp_get_asset_upload_path($vo['url']);?>" />
              </div><?php endforeach; endif; ?>
        </div>
        <div class="new_about_txt">

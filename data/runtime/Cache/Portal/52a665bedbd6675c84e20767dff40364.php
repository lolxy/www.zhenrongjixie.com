...
<div class="clear"></div>
<div id="warp_main">
<?php $slides=sp_getslide("news_ads"); ?>
...
<div id="example-one">
<ul class="nav">
  <li class="nav-one"><a href="<?php echo leuu('list/index',array('id'=>3));?>" 
    <?php if($cat_id == 3): ?>class="current"<?php endif; ?>
    >公司新闻</a></li>
  <li class="nav-one"><a href="<?php echo leuu('list/index',array('id'=>4));?>" 
    <?php if($cat_id == 4): ?>class="current"<?php endif; ?>
    >行业新闻</a></li>
</ul>
<div class="list-wrap">
<ul id="featured">
...
<ul class="article_list">
  <ul class="news_list clearfix">
    <?php $lists = sp_sql_posts_paged_bycatid($cat_id,"order:post_date DESC;",10); ?>
    <?php if(is_array($lists['posts'])): $i = 0; $__LIST__ = $lists['posts'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i; $smeta=json_decode($vo['smeta'], true); ?>
      <li onclick="location='<?php echo leuu('article/index',array('id'=>$vo['object_id'],'cid'=>$vo['term_id']));?>'">
        <div class="left_image">
          <?php if(empty($smeta['thumb'])): ?><img src="/themes/simplebootx/Public/images/default_tupian1.png" alt="<?php echo ($vo["post_title"]); ?>" width="179" height="120"/>
            <?php else: ?>
            <img src="<?php echo sp_get_asset_upload_path($smeta['thumb']);?>" alt="<?php echo ($vo["post_title"]); ?>" width="179" height="120"/><?php endif; ?>
        </div>
        <div class="r_text_box">
          <h5><?php echo ($vo["post_title"]); ?></h5>
          <p class="text"><?php echo ($post_excerpt); ?></p>
          <p class="time"><?php echo date('Y-m-d',strtotime($vo['post_date']));?><span>来源：<?php echo ($vo["post_source"]); ?></span></p>
        </div>
